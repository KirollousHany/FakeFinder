...
import com.example.fakefinder.databinding.FragmentWaitingGenerateBinding
...
@AndroidEntryPoint
class WaitingGenerateFragment : Fragment() {

    private var _binding: FragmentWaitingGenerateBinding? = null
    private val binding get() = _binding!!
    private val waitingGenerateViewModel: WaitingGenerateViewModel by viewModels()
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_waiting_generate, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        _binding = FragmentWaitingGenerateBinding.bind(view)
        audioDownloader = AudioDownloader(requireContext())
        val sourceVoiceUri =
            WaitingGenerateFragmentArgs.fromBundle(requireArguments()).firstVoiceUri
        val targetVoiceUri =
            WaitingGenerateFragmentArgs.fromBundle(requireArguments()).SecondVoiceUri
        waitingGenerateViewModel.generateAudio(sourceVoiceUri, targetVoiceUri, requireContext())
        observers()
        onClicks()
        handleOnBackPressed()
    }
    ...
    private fun observers() {
        waitingGenerateViewModel.validateGenerateRespond.observe(viewLifecycleOwner) {
            url = it.voiceUrl
            time = it.name
            handleMediaHideUnhidden()
        }
        waitingGenerateViewModel.validateResult.observe(viewLifecycleOwner) {
