...
@HiltViewModel
class WaitingGenerateNewViewModel @Inject constructor(
    ...
    private fun detectLanguage(text: String): String {
        ...
        val containsEnglish = englishPattern.containsMatchIn(text)
        val containsArabic = arabicPattern.containsMatchIn(text)

        return when {
            containsEnglish && !containsArabic -> "en" // Only English
            containsArabic && !containsEnglish -> "ar" // Only Arabic
            containsEnglish && containsArabic -> "mixed" // Mixed languages
            else -> "unknown" // Neither English nor Arabic
        }
    }

    fun generateAudio(voiceUri: Uri, text : String, context: Context) {
        val sourceVoice = uriToFile(context, voiceUri)!!
        val language = detectLanguage(text)
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val data = repo.generateNew(sourceVoice,text, language)
                if (data.code() == 200) {
    ...
    private fun uploadVoice(url: String) {
